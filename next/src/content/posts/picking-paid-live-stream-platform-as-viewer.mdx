...
<Figure src={zan_koko_player_iiImg} caption="2023-09-03: KOKO one-man live is just to good I had to forgive Z-aN for their previous issues." />
...
<Figure src={zan_on_firefox_failImg} alt="Z-aN video player fail with `The m3u8 file loaded file` error message" caption="On Firefox, after few mins the video player will error out, needing a page refresh" />

### Trivia

<Figure src={zan_date_byteImg} alt="Showing date input with 'Please enter your date of birth as a single-byte number.' from Z-aN registration flow" caption="Bonus: Z-aN wants me to enter my date of birth as a single-byte number, I have no idea how to convert a date into a single byte." />
